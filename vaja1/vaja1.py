...
def additive_noise(img):
    ...
    #noise = np.zeros((height,width,3), np.int32)
    mean = np.array([0, 0, 0])
...
def generate_ellipse(img):
    ...
    image = cv2.ellipse(img, [random.randint(0, height), random.randint(0, width)], [random.randint(0, height), random.randint(0, width)], random.randint(0, 90), 0, 360, random.randint(0, width), -1)
    return img
...
def generate_image(shepe_type,height,width):
    ...
    img_add = additive_noise(img_add)
    img_mul = multiplicative_noise(img_mul)
    
    return img_add, img_mul
...
while True:
    img_add,img_mul = generate_image(random.randint(0,2),320,320)
    cv2.imshow("img add",img_add)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    cv2.imshow("img mul",img_mul)
    cv2.waitKey(0)
    cv2.destroyAllWindows()